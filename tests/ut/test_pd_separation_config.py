...
def test_pd_separation_config_reads_scheduler_zmq_ports(monkeypatch):
    monkeypatch.setenv(
        "VLLM_ASCEND_PD_SCHEDULER_EDGE_TO_CLOUD_ZMQ_PORT", "6008"
    )
    monkeypatch.setenv(
        "VLLM_ASCEND_PD_SCHEDULER_CLOUD_TO_EDGE_ZMQ_PORT", "6009"
    )
